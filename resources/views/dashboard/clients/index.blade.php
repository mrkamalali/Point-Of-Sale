                            ...
                            <table class="table table-hover">
                                ...
                                @foreach ($clients as $index=>$client)
                                    <tr>
                                        <td>{{ $index + 1 }}</td>
                                        <td>{{ $client->name }}</td>
                                        <td>{{ is_array($client->phone) ? implode($client->phone, '_') : $client->phone  }}</td>
                                        <td>{{ is_array(implode($client->phone, '_')) ?? $client->phone  }}</td>
                                        <td>{{ $client->address }}</td>
                                        <td>
                                            @if(auth()->user()->hasPermission('create_orders'))
                                                <a href="{{ route('clients.orders.create', $client->id ) }}"
                                                   class="btn btn-microsoft btn-sm">@lang('site.add_order')</a>
                                            @else
                                                <a href=""
                                                   class="btn btn-danger btn-sm disabled">@lang('site.add_order')</a>
                                            @endif
                                        </td>
                                        <td>
